...
import { WithdrawBehaviorForNextPlayer } from './WithdrawBehaviorForNextPlayer';
...
describe('WithdrawBehavior', () => {
    it('should withdraw cards', () => {
        ...
        game.players[1].cards = [{ id: 'c-3', behaviors: [], isWild: true }];
        game.activePlayerIndex = 0;

        const behavior = new WithdrawBehaviorForNextPlayer(2, 1);
        const { game: newGame, events } = behavior.execute(game);

        expect(newGame.players[1].cards.length).toEqual(3);
    ...
    it('should withdraw remaining cards if the request cards more than available', () => {
        ...
        expect(newGame.players[1].cards.length).toEqual(2);
